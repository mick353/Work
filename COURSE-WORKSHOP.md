...
Warnings are advisory quality suggestions, not release gates. They remain visible in Review and in the exported validation report, but they do not disable Preview or final outputs. If a reviewer deliberately leaves a warning in place, the optional **Advisory warnings reviewed** checkbox records that decision; it is never a substitute for fixing a genuine error.

New courses begin with the content-review and source-checked dates blank. Enter those dates only after the corresponding review has actually occurred. A missing content-review date is not a drafting blocker; it becomes a release blocker only when status is set to **Available**. The Review page groups issues by authoring step, starts on blockers with groups collapsed, and sends each selected issue back to its relevant field with keyboard focus. An untouched course is labelled **Not started** rather than presented as a failed course. Every deliberate step change starts at and focuses the new heading; on initial load, the first keyboard stop is the **Skip to course editor** link.

**Start blank course** and the sidebar **New course** control reset the current browser draft. **Load draft** and **Clone as new course** can also replace what is currently open. Whenever the current draft contains course work, the Workshop warns before any of those replacement actions and recommends downloading it first; cancelling preserves the current draft unchanged. An untouched starter can start blank, load or clone without an unnecessary warning.

When a valid authored draft is recovered from browser storage after a reload, the Workshop identifies the recovered course and confirms that autosave is active. If a browser-saved draft cannot be opened, autosave pauses and leaves the unreadable copy untouched. The trainer must then load a downloaded draft or explicitly confirm that a new blank/template course may replace the saved copy. The Workshop never silently converts that failure into a new saved starter.
