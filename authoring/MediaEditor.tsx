...
export function MediaEditor({ entry, setEntry, setMessage }: Props) {
  ...
  const importPdf = async (file: File | undefined) => {
    ...
    try {
      ...
      setMessage(`Imported ${file.name}. Review slide titles, extracted text, course-section assignments and image descriptions.`);
    } catch (error) {
  ...
  const importImages = async (files: File[]) => {
    ...
    try {
      ...
      setMessage(`Imported ${files.length} slide image${files.length === 1 ? "" : "s"}. Review their order, titles, course-section assignments and image descriptions.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    } finally {
      setBusy("");
  ...
  const clearDeck = () => {
    if (!window.confirm("Remove the complete source deck from this draft? Section visuals and other course content will remain.")) return;
    setEntry((current) => {
  ...
  return (
    <div className="workspace-stack media-page">
      <div className="page-heading"><span className="eyebrow">6 · Media and source deck</span><h1>Bring the source material and section visuals with the course</h1><p>Imported files are resized, embedded in the draft and carried into every learner export. Nothing is uploaded. PDF pages become slide images; PowerPoint files should first be saved as PDF or exported as images.</p></div>
      <section className="step-connection" aria-label="How this step connects"><span className="connection-symbol" aria-hidden="true">↔</span><div><strong>How this step connects</strong><p>Register sources and course sections first. Imported slides are assigned to both; lesson and field-guide citations then use the slide numbers to open the correct embedded pages for learners.</p></div></section>
      ...
      <section id="media-slides" className="editor-card">
        <header className="card-header"><div><span className="eyebrow">Complete source deck</span><h2>Slides learners can open from citations</h2><p>Choose the initial course section and source before import. You can reassign individual slides afterwards.</p></div>{entry.content.slides.length > 0 && <button type="button" className="text-danger" onClick={clearDeck}><Trash2 size={16} />Remove complete deck</button>}</header>
        <ol className="connection-steps" aria-label="Connect a source deck to course teaching">
          ...
          <li><strong>Import</strong><span>Choose an initial course section and source, then add the PDF or images.</span></li>
          <li><strong>Review</strong><span>Check every slide title, course section, extracted text and image description.</span></li>
        ...
        <div className="media-import-actions">
          <input ref={pdfRef} className="visually-hidden" type="file" accept="application/pdf,.pdf" aria-label="Import a PDF source deck" tabIndex={-1} onChange={(event) => void importPdf(event.target.files?.[0])} />
          ...
          <input ref={imagesRef} className="visually-hidden" type="file" multiple accept="image/png,image/jpeg,image/webp,.png,.jpg,.jpeg,.webp" aria-label="Import source deck slide images" tabIndex={-1} onChange={(event) => void importImages(Array.from(event.target.files ?? []))} />
      ...
      <section id="media-assets" className="editor-card">
        <header className="card-header"><div><span className="eyebrow">Section visuals</span><h2>One useful image or illustration per course section</h2><p>These replace the generic section illustration in the learner course. Add an image only when it helps explain or orient the learner.</p></div></header>
