...
import { formatSlideReference, parseSlideReference, SlideReferencePicker } from "./SlideReferencePicker";
...
export function AdvancedEditor({ entry, setEntry }: Props) {
  ...
  return (
    <div className="workspace-stack">
      ...
      <section className="step-connection" aria-label="How this step connects"><span className="connection-symbol" aria-hidden="true">↔</span><div><strong>How this step connects</strong><p>Cases link back to course sections. Field-guide entries can reuse registered sources and open imported slides. The toolkit, capstone and exemplars become their own learner destinations without duplicating lesson content.</p></div></section>

      <EditorGroup id="advanced-cases" title="Worked cases" description="Show one realistic situation developing across course sections. A corrected case can deliberately expose and repair a weak first attempt." connection="Each case step selects a course section. Learners can move between the case and the teaching it demonstrates." addLabel="Add case" onAdd={addCase}>
