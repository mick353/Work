...
| Package | Stages | Questions | Cards | Source |
|---|---|---|---|---|
| **Product Management Fundamentals** | 9 | 122 | 92 | The 98-slide *Product Management Fundamentals* deck, written by **Simon Morris**, DEWR Digital Experience and Solutions |
...
npm run qa         # 267-check verification suite
...
`scripts/qa.mjs` runs **267 checks** against the real built artefact in a real browser, and writes `qa-report.json`. Playwright and its Chromium are resolved from `node_modules`, so there are no absolute paths.
...
scripts/
  ...
  qa.mjs               The 267-check verification suite
...
Product Management Fundamentals is built from Simon Morris's deck, which supplies the spine. Closure Reports is built on the DTA project closure reporting standard and the Commonwealth assurance and performance frameworks.
