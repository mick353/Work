...
**Every optional section can be absent.** A second package will not have 98 source slides, a capstone and four worked cases. The player was indexing `[0]` on arrays that a smaller package leaves empty, which took the whole app down, and six views rendered blank with no explanation. Each optional section now has an empty state that says what is missing and confirms the rest still works. Proved by registering a deliberately minimal probe package — two stages, no slides, no capstone, no cases — and walking all seventeen views.
...
**The sidebar groups by activity, not by category.** Fourteen destinations plus nine stages was one flat list — 23 buttons, taller than any laptop viewport, with everything weighted equally. They are now four collapsible groups (Learn, Practise, Apply, Reference). *Results* sits under Practise because you read it after drilling, not with the reference material it superficially resembles. Every group starts open — an earlier build collapsed Apply and Reference to save height, and that hid the guide well enough that it could not be found. Groups can still be collapsed by hand, and the choice is remembered.
