...
What it produces
----------------
  public/courses/<course-id>/slides/slide-NN.webp
  src/courses/<course-id>/slides.ts
...
def course_paths(course_id: str) -> tuple[Path, Path, Path]:
    if not re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)*", course_id):
        raise SystemExit("Course id must be a lowercase URL-safe slug.")
    course_dir = ROOT / "src" / "courses" / course_id
    course_source = course_dir / "course.ts"
    if not course_source.exists():
        raise SystemExit(f"No course source found at {course_source}")
    return course_source, course_dir / "slides.ts", ROOT / "public" / "courses" / course_id / "slides"


def stage_ranges(course_source: Path) -> list[tuple[str, int, int]]:
    """
    ...
    """
    source = course_source.read_text(encoding="utf8")
    pattern = re.compile(
        r'id:\s*"([a-z]+)",\s*\n\s*number:\s*\d+,[\s\S]{0,400}?slides:\s*"(\d+)\s*[–—-]\s*(\d+)"'
    )
    found = [(m.group(1), int(m.group(2)), int(m.group(3))) for m in pattern.finditer(source)]
    if not found:
        raise SystemExit(f"No stage slide ranges found in {course_source}")
    return found


def stage_for(number: int, ranges: list[tuple[str, int, int]]) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(
            ["soffice", "--headless", "--norestore",
             f"-env:UserInstallation={tmp_path.as_uri()}/lo-profile",
             "--convert-to", "pdf",
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Usage: python3 scripts/import-slides.py <course-id> <deck.pptx>")
    course_id = sys.argv[1]
    course_source, metadata_file, out_dir = course_paths(course_id)
    ranges = stage_ranges(course_source)
    pptx = Path(sys.argv[2]).expanduser()
    if not pptx.exists():
    ...
    lines = [
        ...
        " *",
        f" * Metadata for {course_id}. Images live beside the course under public/courses.",
    ...
    print(f"  {len(records)} slides -> {out_dir.relative_to(ROOT)} ({total / 1024 / 1024:.2f} MB, "
          f"{total / len(records) / 1024:.0f} KB mean)")
    print(f"  {metadata_file.relative_to(ROOT)} written")
